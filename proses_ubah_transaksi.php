...
if($_POST){
    ...
    if(empty($status)){
        ...
        include "koneksi.php";
            ...
            $update=mysqli_query($conn,"update transaksi set status='".$status."', pembayaran='".$pembayaran."', tanggal_bayar='".$tgl_bayar."' where id_transaksi = '".$id_transaksi."'") or die(mysqli_error($conn));
            if($update){
                echo "<script>alert('Sukses update paket');location.href='histori_transaksi.php';</script>";
            } else {
                echo "<script>alert('Gagal update paket');location.href='ubah_transaksi.php?id=".$id_transaksi."';</script>";
            }
        }
}
